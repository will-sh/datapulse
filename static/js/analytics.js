  ...
  function renderEvents() {
    ...
    const suffix = config.kafkaEnabled
      ...
      : config.posthogEnabled
        ? ""
        : " · local capture";
    count.textContent = `${events.length} AWC interactions captured${suffix}`;

    if (events.length === 0) {
      list.innerHTML =
        '<p class="event-empty">Browse Marketplace, select a Blueprint, or Launch Demo — events appear here in real time</p>';
      return;
    }
