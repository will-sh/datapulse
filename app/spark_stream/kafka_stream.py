...
def _kafka_options() -> tuple[dict[str, str], str, str]:
    ...
    options = {
        ...
        "kafka.ssl.truststore.location": str(kafka_ca),
    ...
    jaas = (
        ...
        f'ssl.truststore.location="{oauth_ca}" ssl.truststore.type=PEM;'
    )
...
def _create_spark_session():
    ...
    options, allowed_urls, dist_files = _kafka_options()
    if not options["kafka.sasl.oauthbearer.client.id"]:
        raise RuntimeError("KAFKA_CLIENT_ID is required")

    builder = SparkSession.builder.appName("datapulse-spark-kafka-consumer")
    builder = builder.config("spark.sql.shuffle.partitions", "1")
    builder = builder.config("spark.driver.extraJavaOptions", allowed_urls)
    builder = builder.config("spark.executor.extraJavaOptions", allowed_urls)
    if dist_files:
        builder = builder.config("spark.yarn.dist.files", dist_files)
...
def start_streaming_worker() -> None:
    try:
        STORE.set_status("creating Spark session")
