  ...
  function renderSessions(rows, summary) {
    ...
    if (!filtered.length) {
      els.sessionsPanel.innerHTML =
        '<p class="empty">No sessions match the current filter. Browse or submit a form on the Producer site, then refresh.</p>';
      return;
    }
  ...
  function renderConverters(rows) {
    if (!rows.length) {
      els.convertersPanel.innerHTML =
        '<p class="empty">No form submissions yet. Submit Schedule a deep dive or the Playground form on the Producer site, then refresh.</p>';
      return;
    }
